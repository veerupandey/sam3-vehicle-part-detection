...
class Trainer:
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        # Always maintain only the latest checkpoint to save disk space
    ...
    def _save_checkpoint(self, checkpoint, checkpoint_path):
        ...
        assert success
        # Remove any previous numbered checkpoints to free disk space
        import glob
        for old_ckpt in glob.glob(os.path.join(self.checkpoint_conf.save_dir, "checkpoint_*.pt")):
            try:
                os.remove(old_ckpt)
            except Exception:
                pass
